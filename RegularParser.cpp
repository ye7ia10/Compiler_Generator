...
void RegularParser::recognizeDashOpp(vector<string>& strVec) {
    if (strVec.size() < 3) return; /* as it does not contain dash operation */
    vector<string>vec;
...
string RegularParser::getOppName(char c) {
    ...
    switch(c) {
        case '*': return "closure op";
        case '+': return "positive closure op";
        case '(': return "open op";
        case ')': return "close op";
        case '|': return "union op";
        case '.': return "concatenate op";
        default: return "ERROR"; /*  this must not happened as it calls only in the previous cases */
    }
...
DataEntry RegularParser::calcExpression(vector<DataEntry>exp) {
    ...
    if(exp.size() == 1 && !exp[0].isOp) return exp[0];
    return error;
}
